...
- Progressive disclosure: SKILL.md stays under ~500 lines — it holds the triage tables, checklists, and commands. Deep material (chronicles, glossaries, worked examples, derivations) goes in `references/*.md` files the skill points to; executable diagnostics go in `scripts/`. "Embed knowledge" means embed it in the skill's directory, not inflate SKILL.md.
...
- Write ONLY inside `.claude/skills/`; the rest of the repo is read-only. Git commits ARE allowed but scoped strictly to `.claude/skills/` paths — stage files explicitly (never `git add -A`/`git add .`), one commit per skill or per phase. No other mutating git commands (no branch changes, resets, or edits outside the skills tree).
...
Fixer applies blocking+important fixes. After the fixer, re-run the FACTUAL reviewer on the files the fixer touched (fixes can introduce new errors); loop until clean.

## Phase 4 — Cold-start acceptance trial (the real exit criterion)
The library is done only when a cheaper model can actually use it. Spawn 2–3 fresh Sonnet-class agents with NO project context beyond the skill library, each given one representative task drawn from real project work — e.g. (a) diagnose a seeded or historical bug using only the debugging playbook, (b) add a config flag end-to-end through change control, (c) answer a "why is it built this way / has this been tried before" question from the archaeology and architecture skills. Score each trial: did the agent find the right skill from its description, follow it without inventing commands or facts, and reach the correct outcome? Every failure is a defect in the LIBRARY, not the agent — trace it to the missing/misleading content, fix, and re-trial that task once.

Then give me: the skill inventory with one-line descriptions, what you verified by spot-check, the cold-start trial results (task, outcome, defects found and fixed), and what remains uncertain.
